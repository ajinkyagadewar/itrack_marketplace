...
/**
 * Enrol buykart external PHPunit tests
 *
...
require_once $CFG->dirroot . '/enrol/buykart/externallib.php';

class enrol_buykart_externallib_testcase extends externallib_advanced_testcase {
	...
	public function test_enrol_users() {
		...
		$roleid = $this->assignUserCapability('enrol/buykart:enrol', $context1->id);
		...
		$this->assignUserCapability('enrol/buykart:enrol', $context2->id, $roleid);
		...
		enrol_buykart_external::enrol_users(array(
		...
		$this->unassignUserCapability('enrol/buykart:enrol', $context1->id, $roleid);
		try {
			enrol_buykart_external::enrol_users(array(
				array('roleid' => 3, 'userid' => $user1->id, 'courseid' => $course1->id),
			));
		...
		$this->assignUserCapability('enrol/buykart:enrol', $context1->id, $roleid);
		...
		// Call for course without buykart instance.
		$DB->delete_records('user_enrolments');
		...
		try {
			...
			$this->fail('Exception expected if course does not have buykart instance');
		} catch (moodle_exception $e) {
