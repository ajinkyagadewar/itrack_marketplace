...
$instance = $DB->get_record('enrol', array('id' => $enrolid, 'enrol' => 'buykart'), '*', MUST_EXIST);
$course = $DB->get_record('course', array('id' => $instance->courseid), '*', MUST_EXIST);
$context = context_course::instance($course->id, MUST_EXIST);

require_login($course);
$canenrol = has_capability('enrol/buykart:enrol', $context);
$canunenrol = has_capability('enrol/buykart:unenrol', $context);
...
if (!$canenrol and !$canunenrol) {
	...
	require_capability('enrol/buykart:unenrol', $context);
}
...
if (!$enrol_buykart = enrol_get_plugin('buykart')) {
	throw new coding_exception('Can not instantiate enrol_buykart');
}

$instancename = $enrol_buykart->get_instance_name($instance);

$PAGE->set_url('/enrol/buykart/manage.php', array('enrolid' => $instance->id));
$PAGE->set_pagelayout('admin');
$PAGE->set_title($enrol_buykart->get_instance_name($instance));
...
$potentialuserselector = new enrol_buykart_potential_participant('addselect', $options);
$currentuserselector = new enrol_buykart_current_participant('removeselect', $options);
...
if ($canenrol && optional_param('add', false, PARAM_BOOL) && confirm_sesskey()) {
	...
	if (!empty($userstoassign)) {
		foreach ($userstoassign as $adduser) {
			...
			$enrol_buykart->enrol_user($instance, $adduser->id, $roleid, $timestart, $timeend);
		}
...
if ($canunenrol && optional_param('remove', false, PARAM_BOOL) && confirm_sesskey()) {
	...
	if (!empty($userstounassign)) {
		foreach ($userstounassign as $removeuser) {
			$enrol_buykart->unenrol_user($instance, $removeuser->id);
		}
    ...
    <tr>
      ...
      <td id="buttonscell">
          <div id="addcontrols">
              ...
              <p><label for="menuroleid"><?php print_string('assignrole', 'enrol_buykart')?></label><br />
