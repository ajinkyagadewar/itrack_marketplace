...
/**
 * Block institution renderer.
 * @package   block_institution
 * @copyright 2010 Moodle Pty Ltd (http://moodle.com)
 * @author    Jerome Mouneyrac
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_institution_renderer extends plugin_renderer_base {
    ...
    public function restore_confirmation_box($filename, $context) {
        ...
        $searchurl = new moodle_url('/blocks/institution/institutioncourse.php',
        array('add' => 1, 'courseid' => $context->instanceid, 'cancelrestore' => 1,
        'sesskey' => sesskey(), 'filename' => $filename));
        $formrestore = new single_button($restoreurl, get_string('dorestore', 'block_institution'));
        $formsearch = new single_button($searchurl, get_string('donotrestore', 'block_institution'));
        return $this->output->confirm(get_string('restorecourseinfo', 'block_institution'), $formrestore, $formsearch);
    }

    /**
     * Display remove institution success message and a button to be redirected to te referer page
     * @param moodle_url $url the page to be redirected to
     * @return string html
     */
    public function remove_success(moodle_url $url) {
        $html = $this->output->notification(get_string('institutionremoved', 'hub'), 'notifysuccess');
        $continuebutton = new single_button($url, get_string('continue', 'block_institution'));
        $html .= html_writer::tag('div', $this->output->render($continuebutton), array('class' => 'continuebutton'));
        return $html;
    }

    /**
     * Display add institution course success message and a button to be redirected to te referer page
     * @param moodle_url $url the page to be redirected to
     * @return string html
     */
    public function save_link_success(moodle_url $url) {
        $html = $this->output->notification(get_string('addedtoblock', 'block_institution'), 'notifysuccess');
        $continuebutton = new single_button($url, get_string('continue', 'block_institution'));
        $html .= html_writer::tag('div', $this->output->render($continuebutton), array('class' => 'continuebutton'));
        return $html;
    }
    ...
    public function next_button($data) {
        $nextlink = html_writer::tag('a', get_string('next', 'block_institution'),
    ...
    public function course_list($courses, $huburl, $contextcourseid) {
        ...
        if (empty($courses)) {
            if (isset($courses)) {
                $renderedhtml .= get_string('nocourse', 'block_institution');
            }
        } else {
            $courseiteration = 0;
            foreach ($courses as $course) {
                ...
                if (!empty($course->courseurl)) {
                    ...
                    $linktext = get_string('visitsite', 'block_institution');
                } else {
                    $courseurl = new moodle_url($course->demourl);
                    $linktext = get_string('visitdemo', 'block_institution');
                }

                $visitlinkhtml = html_writer::tag('a', $linktext, array('href' => $courseurl,
                'class' => 'hubcoursedownload', 'onclick' => 'this.target="_blank"'));
                ...
                $courseuserinfo = get_string('userinfo', 'block_institution', $course);
                if ($course->contributornames) {
                    $courseuserinfo .= ' - ' . get_string('contributors', 'block_institution', $course->contributornames);
                }
                ...
                if (empty($course->coverage)) {
                    ...
                    $coursecontentinfo .= get_string('coverage', 'block_institution', $course->coverage);
                    $coursecontentinfo .= ' - ';
                }
                $coursecontentinfo .= get_string('contentinfo', 'block_institution', $course);
                ...
                $coursefileinfo = get_string('fileinfo', 'block_institution', $course);
                ...
                if (!empty($course->contents)) {
                    ...
                    $blocksandactivities = html_writer::tag('div', get_string('activities', 'block_institution') .
                    " : " . $activitieshtml);

                    //Uncomment following lines to display blocks information
                    //$blocksandactivities .= html_writer::tag('span',
                    //get_string('blocks', 'block_institution') . " : " . $blockhtml);
                }
                ...
                if (!empty($course->outcomes)) {
                    ...
                    $outcomes = get_string('outcomes', 'block_institution', $outcomes);
                }
                $outcomeshtml = html_writer::tag('div', $outcomes, array('class' => 'hubcourseoutcomes'));

                //create additional information html
                $additionaldesc = $courseuserinfohtml . $coursecontentinfohtml . $coursefileinfohtml .
                $blocksandactivities . $outcomeshtml;
                ...
                if ($course->enrollable) {
                    ...
                    $addurl = new moodle_url("/blocks/institution/institutioncourse.php", $params);
                    $addbuttonhtml = html_writer::tag('a',
                    get_string('addtoinstitutionblock', 'block_institution'),
                ...
                if (!$course->enrollable) {
                    ...
                    $downloadurl = new moodle_url("/blocks/institution/institutioncourse.php", $params);
                    $downloadbuttonhtml = html_writer::tag('a', get_string('install', 'block_institution'),
                    array('href' => $downloadurl, 'class' => 'centeredbutton, hubcoursedownload'));
                }

                //Create rating html
                $rating = html_writer::tag('div', get_string('noratings', 'block_institution'), array('class' => 'norating'));
                if (!empty($course->rating)) {
                    ...
                    if ($course->rating->count > 0) {
                        ...
                        $rating = html_writer::tag('li', '', array('class' => 'current-rating',
                        'style' => 'width:' . $size . 'px;'));
                ...
                $coursecomments = html_writer::tag('div', get_string('nocomments', 'block_institution'),
                ...
                if (!empty($course->comments)) {
                    ...
                    $coursecomments = html_writer::tag('div', get_string('comments', 'block_institution', $commentcount),
                    ...
                    $coursecommenticon = html_writer::tag('div', get_string('comments', 'block_institution', $commentcount),
                ...
                $rateandcomment = html_writer::tag('div', html_writer::tag('a',
                get_string('rateandcomment', 'block_institution'), array('href' => new moodle_url($huburl,
                array('courseid' => $course->id, 'mustbelogged' => true)), 'onclick' => 'this.target="_blank"')),
                ...
                $screenshotbuttonsdiv = html_writer::tag('div', $coursescreenshot . $buttonsdiv,
                array('class' => 'courselinks'));

                $coursedescdiv = html_writer::tag('div', $deschtml . $additionaldeschtml .
                $rating . $coursecomments . $rateandcomment, array('class' => 'coursedescription'));
